...
% -- excitatory --
    % dxdt update
    if i > syn_delay/step
        %for Excitatory Population N_E
        nsub = N_E;
        %disp(nsub)
        for ij = 1:floor(sqrt(nsub))
            for ik = 1:floor(sqrt(nsub))
                jk = floor(sqrt(nsub))*(ij-1)+ik;
                %disp(jk)
                dv_Edt(i,jk) = (vrest - v_E(i,jk) + J_EI/C_EI*S_EI(i-syn_delay/step,jk) + J_EE/C_EE*S_EE(i-syn_delay/step,jk) + mew_e  + sigma_e.*(tau_E_m(1,:).^0.5).*whitenoise_E(i,jk))./tau_E_m(1,:);
            end
        end
    ...
    %for Excitatory Population N_E
         ...
         for ij = 1:floor(sqrt(nsub))
            for ik = 1:floor(sqrt(nsub))
                jk = floor(sqrt(nsub))*(ij-1)+ik;
                dv_Edt(i,jk) = (vrest - v_E(i,jk) + J_EI/C_EI*leftover_S_EI(i,jk) + J_EE/C_EE*leftover_S_EE(i,jk) + mew_e  + sigma_e.*(tau_E_m(1,:).^0.5).*whitenoise_E(i,jk))./tau_E_m(1,:);
            end
         end
    ...
    if i > syn_delay/step
        ...
        for ij = 1:floor(sqrt(nsub))
            for ik = 1:floor(sqrt(nsub))
                ...
                dv_Idt(i,jk) = (vrest - v_I(i,jk) + J_IE/C_IE*S_IE(i-syn_delay/step,jk) + J_II/C_II*S_II(i-syn_delay/step,jk) + mew_i + sigma_i.*(tau_I_m(1,:).^0.5).*whitenoise_I(i,jk))./tau_I_m(1,:);
            end
        end
        ...
        for ij = 1:floor(sqrt(nsub))
            for ik = 1:floor(sqrt(nsub))
                jk = floor(sqrt(nsub))*(ij-1)+ik;
                dv_Idt(i,jk) = (vrest - v_I(i,jk) + J_IE/C_IE*leftover_S_IE(i,jk) + J_II/C_II*leftover_S_II(i,jk) + mew_i  + sigma_i.*(tau_I_m(1,:).^0.5).*whitenoise_I(i,jk) )./tau_I_m(1,:);
            end
        end
        ...
        if v_I(i+1,k) >= 20 && v_I(i,k) < 20
            ...
            for j = 1:N_E
                ...
                if S_key_IE(j,k) ~= 0
                    ...
                    % max synaptic weight check
                    if (J_IE*W_IE(i+1,index)) > 2000
                        W_IE(i+1,index) = 2000/J_IE;                   
                    end
                end
            end
